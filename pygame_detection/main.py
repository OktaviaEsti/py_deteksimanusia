...
while running:
    ...
    if sensorON:
        ...
        #draw all of the detected objects 
        map_pts = np.array(FeatureMap.LASERPOINTS)
        hai = FeatureMap.LASERPOINTS
        ...
        while BREAKE_POINT_INO < abs(FeatureMap.NP - FeatureMap.PMIN):
            ...
            #     # print(FeatureMap.CIRCLE_PARAMS)
            #     x_c, y_c, r_c = FeatureMap.CIRCLE_PARAMS
            #     pygame.draw.circle(environment.infomap, COLOR ,(x_c,y_c),r_c,2)
            #     pts_circle = np.array(circle_seedSeg[0])
            #     pts_circle = pts_circle[:,0]
            #     for point in pts_circle:
            #         environment.infomap.set_at((int(point[0]),int(point[1])), (0,255,0))
            #         pygame.draw.circle(environment.infomap,COLOR,(int(point[0]),int(point[1])),2,0)
            #     circless.append((BREAKE_POINT_INO, FeatureMap.CIRCLE_PARAMS))
